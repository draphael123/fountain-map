...
const SERVICE_URLS: Record<Exclude<ServiceType, 'Planning' | 'Async'>, { learnMore: string; getStarted: string }> = {
  TRT: {
...
export function ServiceQuickLinks({ selectedService }: ServiceQuickLinksProps) {
  // Don't show for Planning or Async (no URLs configured yet)
  if (selectedService === 'Planning' || selectedService === 'Async') return null;
